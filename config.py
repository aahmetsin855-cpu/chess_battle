...
# "Базовые" (UI_SCALE=1.0) значения — фактические BOARD_MARGIN_X/Y и
# SIDE_PANEL_WIDTH ниже пересчитываются от них при каждом изменении
# UI_SCALE (см. set_ui_scale). Раньше эти три константы были ЖЁСТКО
# зафиксированы в пикселях независимо от UI_SCALE/логического разрешения
# — при уменьшении логического экрана на Android они съедали растущую
# ДОЛЮ экрана, из-за чего сама доска ужималась сильнее, чем нужно было
# бы. На Android базовые значения намеренно меньше desktop-х — 90px
# верхнего отступа это 17% от логической высоты экрана 540px, тогда как
# на обычном desktop-окне это разумные ~10-12%.
if platform_utils.is_android():
    _BASE_BOARD_MARGIN_X = 16
    _BASE_BOARD_MARGIN_Y = 34
else:
...
# рисовать в это разрешение напрямую значит платить в разы больше за
...
# как в маленькое окно, а выглядит на весь экран. 960x540 — агрессивный
# выбор в пользу производительности (в разы меньше пикселей, чем 720p);
# если резкости не хватит, можно поднять до 1280x720, но помнить, что
# без низкого логического разрешения остальные оптимизации почти
# бесполезны — производительность в приоритете перед чёткостью.
ANDROID_LOGICAL_SIZE = (960, 540)

# Отключает "косметические" эффекты на Android: бевели клеток, виньетку,
# тень под фигурой, стеклянный блик панелей/кнопок, пульсацию выделения.
# Каждый из них по отдельности недорог за счёт кэширования (см. выше), но
# на телефоне с и так ограниченным бюджетом кадра суммарно они всё ещё
# заметны — а без них игра не становится менее играбельной, только чуть
# менее нарядной. На desktop всегда False — там это не нужно.
ANDROID_LITE_VISUALS = platform_utils.is_android()
...
if platform_utils.is_android():
    PARTICLE_COUNT_IMPACT = 0
    PARTICLE_COUNT_DEATH = 0
else:
...
if platform_utils.is_android():
    AI_DIFFICULTY_SETTINGS = {
        "easy":   {"think_time": 0.3, "max_depth": 2},
        "normal": {"think_time": 0.6, "max_depth": 3},
        "hard":   {"think_time": 1.5, "max_depth": 5},
    }
else:
    AI_DIFFICULTY_SETTINGS = {
...
# ---------------------------------------------------------------------------
# На Android анимации и паузы между действиями ИИ урезаны в разы — на
# телефоне и так каждый кадр дороже, тратить ещё и время ХОДА(!) на
# растянутую анимацию, пока рендер и так на пределе, только усугубляет
# ощущение тормозов. Плюс пауза после каждого атомарного действия ИИ
# (0.18с) умножается на число действий за ход — на Android это ощутимая
# добавка к субъективной задержке ответа игры на действия игрока.
if platform_utils.is_android():
    MOVE_ANIMATION_TIME = 0.07
    ATTACK_ANIMATION_TIME = 0.06
    DEATH_ANIMATION_TIME = 0.08
    IMPACT_FLASH_TIME = 0.05
    AI_STEP_PAUSE = 0.02
    DEFAULT_ANIMATION_SPEED = 2.0
else:
